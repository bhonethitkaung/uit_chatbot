...
print("total words after nltk word tokenizatoin is : ", len(words)) #46
labels = sorted(labels)
print("sorted labels", labels)
print("total labels after nltk word tokenizatoin is : ", len(labels)) #6
print("total questions to train is: ", len(docs_x)) #26
#Bag of Words
training = []
output = []
...
print("training is ", training)
print("output is ", output)
print("numpy training is ", len(training)) #26
print("numpy output is ", len(output)) #26

print("length of training is", len(training[0]))  #46
print("length of output is", len(output[0]))  #6


# #setup the model (feed-forward neural network with two hidden layers)
# tensorflow.reset_default_graph()
#
# net = tflearn.input_data(shape=[None, len(training[0])]) #input shape ko define lote tl. each training input is same length.
